...
const ConnectWallet = ({ connecting, currentAccount, connectWallet, loadingProfile, profileFound, profile }) => {
  ...
  const renderProfile = () => {
    if (loadingProfile) {
      ...
      if (!profileFound) {
        return <p className="bg-green-500 flex justify-center items-center rounded-lg p-1 px-2 mr-5">No Lens Profile Found</p>;
      } else {
        return (
          <div className="flex bg-transparent rounded-lg">
            <img
              className="border-white border rounded-lg w-12 sm:w-16 lg:w-20 ml-auto mr-2 hover:border-green-500"
              src={fixUrl(profile.picture.uri ? profile.picture.uri : profile.picture.original.url)}
              alt="profile-image"
            />
            <div className="sm:grid sm:grid-cols-1 sm:gap-y-4 grid grid-cols-1 gap-y-2">
              <p className='text-white flex-col text-sm sm:text-xl items-center mr-2 flex hover:text-green-500'>{profile.handle.slice(0, 11)}</p>
