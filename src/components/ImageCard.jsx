...
function ImageCard({ imageSrc, imageText }) {
  return (
    ...
    <div className="group w-[300px] h-[300px] m-auto rounded-lg cursor-pointer">
      <div className='w-full h-full shadow-xl transition-all duration-400 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)] relative'>
        <div className="absolute inset-0">
          <img className="h-full w-full rounded-xl object-cover shadow-xl shadow-black/40" src={imageSrc} alt="" />
        </div>
        ...
        <div className="absolute inset-0 h-full w-full rounded-xl bg-black/80 px-12 text-center text-slate-200 [transform:rotateY(180deg)] [backface-visibility:hidden]">
          <div className="flex min-h-full flex-col items-center justify-center">
            <h1 className="text-3xl font-bold">Image Text</h1>
            <p className="text-lg">Title</p>
            <p className="text-base">Description goes here.</p>
          </div>
        </div>
